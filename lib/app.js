...
function createApp() {
  ...
  app.post('/convert/:command(*)',
    ...
    wrap(async (req, res, next) => {
      ...
      // Kill the conversion 1 second before the request ends
      // in order to respond with the reason
      const requestTimeout = req.get('Request-Timeout') || 0;
      options.timeout = requestTimeout > 1 ? requestTimeout - 1 : requestTimeout;
      ...
      res.sendFile(outputFile, function() {
        rimraf(outputFile, function () { console.log("'" + outputFile + "' removed"); });
        res.end();
      });
    })
  );
